...
function JPhaseRetrieval_Exp2_LRBFGSvsWF_noise(tau)
...
#	toll = 1e-5
	...
	# the results of first time is useless.
	JPhaseRetrievalLRBFGSvsFlow(nis[1], 1*7777, pp, ll, toll, tau)
#	tau = 0

	num = 10
	data = zeros(4, length(nis)*2, num)

	for i in 1 : length(nis)
		for j in 1 : num
			println(j)
			# WFt, WFfft, WFMM, Rt, RFFT, RMM
			(data[1, 2*i-1, j], data[2, 2*i-1, j], data[3, 2*i-1, j], data[4, 2*i-1, j], data[1, 2*i, j], data[2, 2*i, j], data[3, 2*i, j], data[4, 2*i, j]) = JPhaseRetrievalLRBFGSvsFlow(nis[i], j*7777, pp, ll, toll, tau)
		end
	end
	println(pwd())
	writedlm("data_exp2_l6_" * sprintf1("%f", tau) * ".txt", data)

	tab = zeros(4, length(nis)*2)
	for i = 1 : length(nis)*2
		tab[1, i] = sum(data[1, i, :]) / num
		tab[2, i] = sum(data[2, i, :]) / num
		tab[3, i] = sum(data[3, i, :]) / num
		tab[4, i] = sum(data[4, i, :]) / num
	end

	open("./Paper_LRBFGS_vs_WF_" * sprintf1("%f", tau) * ".txt", "w") do f
...
function JPhaseRetrievalLRBFGSvsFlow(ni, randseed, pp, ll, toll, tau)
	global n1 = ni
	global n2 = ni
	global n = n1 * n2
	global l = ll
...
#	global masks = octanaryrandom(n1, n2, l)
	...
	for i in 1 : l
		...
		btrue[(i - 1) * n + 1 : i * n] = conj(zi) .* zi;
		b[(i - 1) * n + 1 : i * n] = conj(zi) .* zi + tau * randn(n, 1);
	end
	SNR = 10 * log(norm(btrue) / norm(btrue - b)) / log(10)
	println("SNR:", SNR)

	# initial iterate for Riemannian approach.
	(initialX::Array{Complex{Float64}, 2}, Rinittime) = InitialIterate(p, 10, masks, n, l, m, b)
	...
	p = 1
	(initialX, WFinittime) = InitialIterate(p, Int64(floor(50 / p)), masks, n, l, m, b)
	...
	return (WFtime + WFinittime, nfftWF, 0, WF, Rtime + Rinittime, nfftrank1, nMM, R1)
end
