...
function JPhaseRetrieval_Exp0_global()
	deltas = [0.95]
	ls = [2, 3, 4, 5, 6, 7, 8]
	#JTestPhaseRetrieval(1, deltas[1], 1);# the results of first time are useless.
	
	num = 100
	data = zeros(2, length(ls), num)
	for j in 1 : length(ls)
		for i in 1 : num
			err= 1#JTestPhaseRetrieval(1, deltas[1], i, ls[j])
			data[1, j, i] = ((err < 1e-5) ? 1 : 0)
		end

		for i in 1 : num
			err= JTestPhaseRetrieval(2, deltas[1], i, ls[j])
			data[2, j, i] = ((err < 1e-5) ? 1 : 0)
		end
	end
	
	tab = zeros(2, length(ls))
	...
	println(pwd())
	open("./Paper_1vs2_tab.txt", "w") do f
		for i in 1 : size(tab, 1)
			for j in 1 : size(tab, 2)
				tabstr = sprintf1("%3.2f", tab[i, j])
				if(j < size(tab, 2))
					write(f, "\$" * tabstr * "\$ & ")
				else
					write(f, "\$" * tabstr * "\$ ")
...
#--------------------main test function-----------------------------------------
function JTestPhaseRetrieval(pp, ddelta, randseed, ll)
	global n1 = 32
	global n2 = 32
	global n = n1 * n2
	global l = ll
...
#	global masks = octanaryrandom(n1, n2, l)
	...
	while true
		...
		(FinalIterate, fv, gfv, gfgf0, iteri, nfi, ngi, nRi, nVi, nVpi, nHi, ComTime, funs, grads, times) = DriverJuliaOPT(Handles, Sparams, Mparams, HasHHR, initialX)
		...
		time += ComTime; nf += nfi; ng += ngi; nR += nRi; nH += nHi; nV += nVi; nVp += nVpi; Iter += iteri; 
		nfft += (nfi + ngi) * l * p; nn += 2 * ngi * p^2 + 6 * p^2 * (iter) + ((p > 1) ? 2 * (iter - 1) * p^2 : 0);
		if(p == 1)
			break;
		end
		(U, S) = svd(FinalIterate)
		initialX = reshape(U[:, 1] * S[1], n, 1)
...
#		end
	...
	println("Num of total iter.:", Iter, ", total time:", time, ", nf:", nf, ", ng:", ng, ", nH:", nH, ", nV:", nV, ", nR:", nR,
			", nfft:", nfft)
	println("err:", vecnorm(vphase - FinalIterate * (FinalIterate'*vphase) / norm(FinalIterate'*vphase)) / vecnorm(vphase))
	return vecnorm(vphase - FinalIterate * (FinalIterate'*vphase) / norm(FinalIterate'*vphase)) / vecnorm(vphase)
end
