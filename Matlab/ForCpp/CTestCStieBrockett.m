function MTestStieBrockett()
    ...
    n = 4;
    p = 2;
    Xinitial = orth(complex(randn(n, p), randn(n, p)));
    B = complex(randn(n, n), randn(n, n));
    B = B + B';
    
    D = complex(ones(p, 1), zeros(p, 1));%(p:-1:1)'; %
...
%     SolverParams.IsCheckGradHess = 1;
    HasHHR = 0;
    paramset = 1;
    [Xopt, f, gf, gfgf0, iter, nf, ng, nR, nV, nVp, nH, ComTime, funs, grads, times] = TestCStieBrockett(B, D, Xinitial, HasHHR, paramset, SolverParams);
