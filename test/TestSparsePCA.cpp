...
void testSparsePCA(double *B, double *Dsq, integer p, integer n, integer r, double mu, double *X, double *Xopt)
{
	...
	LRBFGSLPSub *LRBFGSLPSubsolver = new LRBFGSLPSub(&Prob, &ObliqueX);
	LRBFGSLPSubsolver->Debug = FINALRESULT;
	...
	LRBFGSLPSubsolver->Accuracy = 1e-5;
	...
	if (LRBFGSLPSubsolver->Getnormgf() < 1e-6)
		printf("SUCCESS!\n");
	else
		printf("FAIL!\n");
